...
let score = 0;
let itemX, itemY;
let itemSpeed = 2.0; // 再度微調放慢，確保絕對反應得過來
...
function setup() {
    ...
    resetItem();
    bucketX = width / 2;
    // 【核心修正】將垃圾桶的位置往上提，強迫玩家把手擺在鏡頭中央，防止手部移出鏡頭盲區
...
function processHandTracking() {
    if (hands.length > 0) {
        ...
        // 【重大修正點】針對手機微縮螢幕，大幅放寬判定閾值，讓操作反應極度靈敏
        let closeThresh = 35; // 只要兩指距離小於 35 像素即判定捏緊
        let openThresh = 55;  // 超過 55 像素即算作張開手
...
function resetItem() {
    ...
    itemSpeed = 2.2 + (score * 0.01); // 穩定掉落流暢度
...
function manageFallingObjects() {
    ...
    strokeWeight(1.2);
    fill(5, 10, 25, 240);
    let cardW = width * 0.32; // 稍微加寬卡片，在手機上更好對齊
    ...
    // 擴大碰撞包圍盒判定
    let withinX = (itemX > bucketX - bucketWidth / 2 - 20) && (itemX < bucketX + bucketWidth / 2 + 20);
